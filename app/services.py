...
def fetch_prices_by_ticker(ticker: str) -> List[Dict]:
    ...
    Returns:
        List[Dict]: Список словарей с ключами 'ticker', 'price', 'timestamp',
    ...
    return [{"ticker": r[0], "price": r[1], "timestamp": r[2]} for r in rows]
...
def fetch_prices_by_date(ticker: str, from_ts: int, to_ts: int) -> List[Dict]:
    ...
    Args:
        ticker (str): Символ актива.
        from_ts (int): Начало интервала (timestamp в uс).
        to_ts (int): Конец интервала (timestamp в uс).
...
def save_price(ticker: str, price: float, timestamp: int) -> None:
    """
    ...
    query = """
            INSERT INTO index_prices (ticker, price, timestamp)
            VALUES (%s, %s, %s)
            """
    params = (ticker, price, timestamp)
